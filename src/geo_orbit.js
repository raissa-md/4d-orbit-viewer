...
import { AS2RD } from './Orbit.js'
import { J2000_JD } from './Orbit.js'
import { DAYS_PER_CENT } from './Orbit.js'
...
class Geo
    ...
    /* GEI2000_to_GEI_OLD and GEI_to_GEI2000_OLD are the original implementations of GEI2000_to_GEI
    // and GEI_to_GEI2000.  They are retained here for reference.
    GEI2000_to_GEI_OLD (gei2000, time) 
        {
        //   INPUT:
    ...
    */
    ...
    // ---------------------------------------------------------------------------
    //  Replacement for GEI2000_to_GEI / GEI_to_GEI2000 in geo_orbit.js
    //
    //  Precession model: IAU 2006 (P03), Capitaine, Wallace & Chapront,
    //  A&A 412, 567 (2003), eq. 45.  Supersedes the IAU 1976 model used by the
    //  original Fortran.  Same three-polynomial form, same runtime cost.
    //
    // --------------------------------------------------------------------------
    GEI2000_to_GEI (gei2000, time)
        {
        //   INPUT:
        //      gei2000 = X,Y,Z mean equator and equinox of J2000
        //      time    = POSIX time in milliseconds
        //   OUTPUT:
        //      GEI     = X,Y,Z mean equator and equinox of date
        //
        //   CONVERTS GEI2000 X,Y,Z COORDS TO GEI X,Y,Z COORDS.
        //
        //  J2000: Geocentric Equatorial Inertial for epoch J2000.0 (GEI2000), also known as Mean Equator
        //  and Mean Equinox of J2000.0 (Julian date 2451545.0 TT (Terrestrial Time), or 2000 January 1 noon TT,
        //  or 2000 January 1 11:59:27.816 TAI or 2000 January 1 11:58:55.816 UTC.) This system has X-axis
        //  aligned with the mean equinox for epoch J2000; Z-axis is parallel to the rotation axis of
        //  the Earth, and Y completes the right-handed orthogonal set.

        //  GEI: Geocentric Equatorial Inertial system. This system has X-axis pointing from the Earth 
        //  toward the first point of Aries (the position of the Sun at the vernal equinox). This direction 
        //  is the intersection of the Earth's equatorial plane and the ecliptic plane and thus the X-axis 
        //  lies in both planes. The Z-axis is parallel to the rotation axis of the Earth, and y completes
        //  the right-handed orthogonal set (Y = Z * X). Geocentric Inertial (GCI) and Earth-Centered 
        //  Inertial (ECI) are the same as GEI.

        //  AI code generated by Claude Opus. 

        const cent = (JD (time) - J2000_JD) / DAYS_PER_CENT
        ...
        //  IAU 2006 / P03 equatorial precession angles, arcseconds.
        const zetaA  = (  2.5976176
                       + 2306.0809506 * cent
                       +    0.3019015 * t2
                       +    0.0179663 * t3
                       -    0.0000327 * t4
        ...
        const thetaA = ( 2004.1917476 * cent
                       -    0.4269353 * t2
                       -    0.0418251 * t3
                       -    0.0000601 * t4
                       -    0.0000001 * t5) * AS2RD
        ...
        a [0] [0] = -Math.sin (zetaA) * Math.sin (zA) + Math.cos (zetaA) * Math.cos (zA) * Math.cos (thetaA)
        a [0] [1] = -Math.cos (zetaA) * Math.sin (zA) - Math.sin (zetaA) * Math.cos (zA) * Math.cos (thetaA)
        a [0] [2] = -Math.cos (zA) * Math.sin (thetaA)

        a [1] [0] =  Math.sin (zetaA) * Math.cos (zA) + Math.cos (zetaA) * Math.sin (zA) * Math.cos (thetaA)
        a [1] [1] =  Math.cos (zetaA) * Math.cos (zA) - Math.sin (zetaA) * Math.sin (zA) * Math.cos (thetaA)
        a [1] [2] = -Math.sin (zA) * Math.sin (thetaA)

        a [2] [0] =  Math.cos (zetaA) * Math.sin (thetaA)
        a [2] [1] = -Math.sin (zetaA) * Math.sin (thetaA)
        a [2] [2] =  Math.cos (thetaA)
    ...
    GEI_to_GEI2000 (gei, time)
        ...
        //      GEI2000 = X,Y,Z mean equator and equinox of J2000
        //
        //   CONVERTS GEI X,Y,Z COORDS TO GEI2000 X,Y,Z COORDS.
        //
        //   Precession is a rotation, so this matrix is the transpose of the
        //   one in GEI2000_to_GEI.  Only the row/column indices differ.
        ...
        a [0] [0] = -Math.sin (zetaA) * Math.sin (zA) + Math.cos (zetaA) * Math.cos (zA) * Math.cos (thetaA)
        a [1] [0] = -Math.cos (zetaA) * Math.sin (zA) - Math.sin (zetaA) * Math.cos (zA) * Math.cos (thetaA)
        a [2] [0] = -Math.cos (zA) * Math.sin (thetaA)

        a [0] [1] =  Math.sin (zetaA) * Math.cos (zA) + Math.cos (zetaA) * Math.sin (zA) * Math.cos (thetaA)
        a [1] [1] =  Math.cos (zetaA) * Math.cos (zA) - Math.sin (zetaA) * Math.sin (zA) * Math.cos (thetaA)
        a [2] [1] = -Math.sin (zA) * Math.sin (thetaA)

        a [0] [2] =  Math.cos (zetaA) * Math.sin (thetaA)
        a [1] [2] = -Math.sin (zetaA) * Math.sin (thetaA)
        a [2] [2] =  Math.cos (thetaA)
    ...
    GEI_to_GEO (gei, gmst)
        ...
        //  GEO: Geographic coordinate system. This system is defined so that its X-axis is in the Earth's
        ...
        //  and its Y-axis completes a right handed orthogonal set (Y = Z * X).
